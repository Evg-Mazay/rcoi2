...
def test_request_new_item(fresh_database):
    ...
    with app.test_client() as test_client:
        response = test_client.post("/warehouse", json=TEST_ORDER)
...
def test_request_get_info(fresh_database):
    refresh_items_in_db()
    with app.test_client() as test_client:
        order_item_id = json.loads(test_client.post("/warehouse", json=TEST_ORDER).data)["orderItemUid"]
        response = test_client.get("/warehouse/" + order_item_id)
        assert response.status_code == 200
        response = json.loads(response.data)
        assert response["size"] == TEST_ORDER["size"]
        assert response["model"] == TEST_ORDER["model"]
...
def test_request_warranty(fresh_database):
    ...
    with app.test_client() as test_client:
        with requests_mock.Mocker(real_http=True) as m:
            m.post(
                re.compile("/warranty/item-1/warranty"),
                json={"warrantyDate": "2020-11-11", "decision": "FIXING"}
            )
            response = test_client.post("/warehouse/item-1/warranty", json={"reason": "Broken"})
...
def test_request_remove_item(fresh_database):
    ...
    with Session() as s:
        s.add(OrderItem(item_id=1, order_item_uid="item-1", order_uid='1-1-1'))
    with app.test_client() as test_client:
        response = test_client.delete("/warehouse/item-1")
